...
export default function AccountDetailPage() {
  ...
  const orders = React.useMemo(() => generateOrdersFor(company), [company]);
  ...
  // Recommended action — synthesised from state
  ...
  return (
    ...
    >
      ...
      {/* Two-column: signals + revenue chart */}
              ...
              <ul className="space-y-2">
                {company.top3CrossSellSkus.map((code) => (
                  <li key={code} className="text-sm">
                    ...
                    <div>{company.top3CrossSellSkus.includes(code) ? code : code}</div>
                  </li>
                ))}
              </ul>
            )}
...
function Signal({ label, good, detail, positive }: { label: string; good: boolean; detail: string; positive?: boolean }) {
  // `positive` flips the colouring for things like "buyer intent active = good"
  // `good` is the default — true means green dot, false means amber dot
  const dot = good
    ? "bg-emerald-500"
    : positive === true
      ? "bg-muted-foreground"
