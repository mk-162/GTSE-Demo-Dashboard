...
import { lapsedTier } from "@/lib/lapsed-tier";
...
function reactivationLikelihood(c: { healthScore: number; lifetimeRevenue: number; daysSinceLastOrder: number; lifetimeOrders: number }): number {
  ...
  score += Math.min(c.lifetimeRevenue / 1000, 40);
...
export function LapsedView({ region, insight, all, nameToIdMap }: Props) {
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {[
              ...
              const pct = lapsedPool > 0 ? (row.value / lapsedPool) * 100 : 0;
              return (
                <div key={i}>
