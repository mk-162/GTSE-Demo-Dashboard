...
function ReorderTable({ rows, region }: { rows: Company[]; region: "UK" | "US" }) {
  return (
    <Card>
...
export default function ReorderPage() {
  ...
  const insight = insightOf(region, "reorder_urgency")!;
  const all = region === "UK" ? COMPANIES_UK : COMPANIES_US;

  // Bucket companies by predicted reorder timing
  ...
  for (const c of all) {
    if (c.lifetimeOrders < 3) continue; // only customers with established cadence
    const dvp = daysVsPredicted(c);
  ...
  // Forecast — bucket reorders by week for next 8 weeks
